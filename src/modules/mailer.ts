...
class Mailer {
  ...
  async sendNewUserEmail(name: string, email: string, link: string) {
    ...
    await this.execute(email, "Bem-vindo(a).", variables, templatePath, text)
      .then(() => {
        return true;
      })
      .catch((err) => {
  ...
  async sendUserConfirmedResetPassword(name: string, email: string) {
    ...
    const templatePath = resolve(
      ...
      "templates",
      "emails",
      "confirmedResetUserPassword.hbs"
    );

    const text = `A sua senha de acesso ao aplicativo foi alterada.`;

    await this.execute(email, "Senha alterada.", variables, templatePath, text)
      .then(() => {
        return true;
      })
      .catch((err) => {
